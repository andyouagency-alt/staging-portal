...
// Baut den Prompt je nach Modus und Oberflächen-Wünschen – kurz und positiv
function buildPrompts(mode, floorKey, wallKey, userPrompt) {
  ...
  if (mode === "rohbau") {
    p += "Renovate the raw surfaces: " + (floor || "high-end flooring matching the style") + " as new flooring, "
      ...
      + "The room structure stays identical to the input photo: same camera angle and framing, same walls, "
      + "same open passages and doorways with the adjacent space visible through them, same windows with the same frames and the same view outside, same proportions. ";
  } else {
    p += "Keep the room itself identical to the input photo: same camera angle and framing, same walls, "
      + "same open passages and doorways with the adjacent room visible through them, same windows with the same frames and the same view outside, "
      + (floor ? "" : "same floor, ")
  ...
  p += "Add freestanding furniture and decor: " + userPrompt.trim() + " ";
  p += "Place all furniture on the open floor area in the middle of the room, leaving a generous clear zone in front of every window, glass door, doorway and walkway. "
    ...
    + "Realistic scale and shadows, staged exactly like a professional home stager for a luxury property listing. "
