...
// Baut den Prompt je nach Modus, Oberflächen-Wünschen und Immobilien-Key-Facts – kurz und positiv
function buildPrompts(mode, floorKey, wallKey, userPrompt, propKey, sqm, facts) {
  ...
  if (mode === "rohbau") {
    ...
    p += "New interior: " + userPrompt.trim() + " ";
  } else {
  ...
  // Immobilien-Kontext aus dem Exposé
  if (prop || sqmNum || factsTxt) {
...
async function verifyStructure(originalDataUri, resultUrl) {
  try {
    const r = await fetch("https://fal.run/fal-ai/any-llm/vision", {
      ...
      body: JSON.stringify({
        ...
        prompt: "Image 1 is the original room photo. Image 2 is a virtually staged version of it. " +
          "Ignore furniture, decor, wall paint and floor material — those may differ. " +
...
app.post("/api/stage", requireCustomer, async (req, res) => {
  try {
    ...
    const built = buildPrompts(mode, floor, wall, prompt, propType, sqm, facts);
    ...
    for (let round = 0; round < MAX_ROUNDS && need > 0; round++) {
      ...
      const checks = await Promise.all(urls.map(u => verifyStructure(imageDataUri, u)));
      lastFailed = [];
    ...
    logEvent("visualisierung", passed.length + " Variante(n), Modus " + (mode || "moebeln") + ", Format " + (format || "original") + (warnings ? ", " + warnings + " mit Warnung" : ""), -passed.length);
